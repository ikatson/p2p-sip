# Copyright (c) 2007, Kundan Singh. All rights reserved.
...
from p2psip.app.dummycrypto import sign, verify, PublicKey, PrivateKey, extractPublicKey
...
H = lambda x: long(hashlib.sha1(x).hexdigest(),
                   16) # the hash function used in this implementation of the DHT.
Hsize = hashlib.sha1(
    'something').digest_size # this is the global size of the hash function result.
Hmod = 2 ** (
    Hsize * 8) # modulus for the hash space, e.g., for SHA1 it is 2*160 in binary.
...
int2bin = lambda x: (''.join(chr(a) for a in [((x >> c) & 0x0ff) for c in
                                              xrange((Hsize - 1) * 8, -8,
                                                     -8)])) if x is not None else '\x00' * Hsize
dig2int = lambda x, b, d=0: reduce(lambda m, n: (m << b) + n, x, 0)
int2dig = lambda x, b, d: [int((x >> c) & ((1 << b) - 1)) for c in
                           xrange(b * (d - 1), -b, -b)]

distance = lambda a, b: min((Hmod + b - a) % Hmod, (Hmod + a - b) % Hmod)
inrange = lambda L, H, a: L <= (a if L <= a else a + Hmod) <= (
    H if L <= H else H + Hmod) # whether a is in range [L,H] modulo
...
findNode = lambda id, nodes: find(lambda x: x.guid == id,
                                  nodes) # find a node from sequence of nodes using the guid of the node
...
_attrName = map(lambda w: (w[0].strip(), map(str.strip, w[1].split(','))),
                map(lambda z: z.split(':'),
                    filter(lambda y: y, map(str.strip, _attrs.split('\n')))))
_attrType = dict(sum(map(lambda x: map(lambda y: (y, x[0]), x[1]), _attrName),
    [])) # dict of attr=>type
_attrList = sum(map(lambda x: x[1], _attrName), []) # list of all attr
_request = map(str.strip,
               'Ack, Discover, Proxy, Join, RoutingTable, LeafSet, Lookup, Neighbor, Ping, Route, Replicate, ReplicaSet, Hash, Keys, Data, Put, Get, Connect, Datagram'.split(
                   ','))
...
def _attr2bin(attr):
    ...
    try:
        return struct.pack('!H', 0x08000 | _attrList.index(attr))
    except:
        pass # throws an error if not found in _attrList
    try:
        r, t = attr.split(':');
        r, t = _request.index(r), _type.index(t)
        if r < 0 or t < 0: raise ValueError, 'invalid attribute', attr
        return struct.pack('!H', ((t << 12) & 0x7000) | (r & 0x0fff))
    except:
        raise ValueError, 'invalid attribute name %r' % attr
...
def _bin2attr(value):
    ...
    try:
        if value & 0x8000:
            return _attrList[value & 0x7fff]
        else:
            return _request[value & 0x0fff] + ':' + _type[
                (value & 0x7000) >> 12]
    except:
        raise ValueError, 'invalid attribute value 0x%x' % value
...
class Message(dict):
    ...
    def __init__(self, raw=None, **kwargs):
        ...
        if not raw:
            ...
            if struct.unpack('!H', raw[:2])[
                0] != version: raise ValueError, 'invalid version'
            self.decode(raw[2:])
    ...
    def __repr__(self):
        ...
        return '<Message name=%s %s>' % (self.name, ' '.join(
            map(lambda x: '%s=%r' % (x[0], x[1]),
                filter(lambda y: y[0] != 'name', self.items()))))
    ...
    def encode(self):
        ...
        for name, elem in filter(lambda x: x[0] != 'name', self.items()):
            ...
            t = _attrType[name]
            if t == 'bool':
                value = struct.pack('!B', elem and 1 or 0)
            elif t == 'int32':
                value = struct.pack('!I', elem)
            elif t == 'int64':
                value = struct.pack('!II', int(elem / (2 ** 32)),
                                    int(elem % (2 ** 32)))
            elif t == 'hash':
                value = int2bin(elem)
            elif t == 'node':
                ...
                value = struct.pack('!H', len(str(elem))) + str(elem)
            elif t == 'bin':
                value = struct.pack('!H', len(str(elem))) + str(
                    elem) # ; print 'name=%r value=%r elem=%r len=%r l=%r'%(name, value, elem, len(str(elem)), struct.pack('!H', len(str(elem))))
            elif t == 'list': # in ('nodes', 'neighbors', 'leafset', 'vals', 'path', 'keyss'): # list
                value = struct.pack('!H', len(elem))
                for e in elem:
                    if name in ('nodes', 'neighbors', 'leafset', 'path'):
                        v = str(e)
                    elif name in ('keyss', 'vals'):
                        v = str(e)
                    else:
                        raise ValueError, 'invalid list type', name
                    value += struct.pack('!H', len(v)) + v
            else:
                raise ValueError, 'invalid element type %r' % t
            if _debug: print 'name=%r elem=%r value=%r len=%r k=%r' % (
                name, elem, value, struct.pack('!H', len(value)), k)
    ...
    def decode(self, value):
        ...
        self['name'], value, remaining = _bin2attr(name), value[4:4 + l], value[
                                                                          4 + l:]
        while len(value) > 0:
            ...
            t = _attrType[name]
            if t == 'bool':
                self[name] = (struct.unpack('!B', elem)[0] != 0)
            elif t == 'int32':
                ...
                self[name] = Node(value=elem)
            elif t == 'msg':
                self[name] = Message(raw=elem)
            elif t == 'str':
                ...
                l = struct.unpack('!H', elem[:2])[0];
                elem = elem[2:2 + l]
                self[name] = elem if name in ('Kp', 'sigma', 'value') else (
                    Key(value=elem) if name == 'key' else Value(value=elem))
            elif t == 'list':
                ...
                for i in xrange(0, count):
                    ...
                    if name in ('nodes', 'neighbors', 'leafset', 'path'):
                        self[name].append(Node(value=v))
                    elif name in ('keyss'):
                        self[name].append(Key(value=v))
                    elif name in ('vals'):
                        self[name].append(Value(raw=v))
                    else:
                        raise ValueError, 'invalid list name', name
            else:
                raise ValueError, 'invalid element type %r' % t
...
class Node(object):
    ...
    def __init__(self, **kwargs):
        ...
        if 'value' in kwargs: # parse from string 
            ...
            ip, self.port, self.type, self.guid = struct.unpack(
                '!4sHB%ds' % Hsize, value)
        ...
        self._str = struct.pack('!4sHB%ds' % Hsize, ''.join(
            [chr(int(x)) for x in self.ip.split('.')]), self.port, self.type,
                                int2bin(self.guid))
        self._repr = '<node ip=%r port=%r type=%r guid=%r>' % (
            self.ip, self.port, self.type, self.guid)
        self._hash = int(self.guid).__hash__() if self.guid else str(
            str(self.type) + ':' + self.ip + ':' + str(self.port)).__hash__()

    def __cmp__(self, other):
        if id(self) == id(other):
            return 0 # a shortcut to compare identical objects
        elif self.guid and other.guid:
            return cmp(self.guid, other.guid)
        else:
            return cmp(self.ip, other.ip) or cmp(self.port, other.port) or cmp(
    ...
    @property
    def hostport(self):
        return self.ip and (self.ip + ':' + str(self.port)) or ''
...
class Network(object):
    ...
    def __init__(self, queue=None):
        ...
        self.node = Node(ip=ip, port=port, type=socket.SOCK_DGRAM,
                         guid=H(ip + ':' + str(port)))
    ...
    def send(self, msg, node, timeout=None):
        ...
        if self.sock:
            try:
                ...
                data = str(msg)
                if _debug: print self.name, 'sending %d bytes %s=>%s: %r' % (
                ...
                if timeout is not None:
                    hash = H(
                        data)    # hash property to associate the ack to the data request.
                    ack = yield self.get(
    ...
    def recv(self, timeout=None, maxsize=1500):
        ...
        if self.sock:
            data, remote = yield multitask.recvfrom(self.sock, maxsize,
                                                    timeout=timeout)
            lastnode = Node(ip=remote[0], port=remote[1],
                            type=socket.SOCK_DGRAM,
                            guid=H(remote[0] + ':' + str(remote[1])))
    ...
    def receiver(self):
        ...
        while True:
            try:
                ...
                continue # ignore it
            if _debug: print self.name, 'received %s=>%s: %r' % (
            ...
            if 'ack' in msg and msg.name != 'Ack:Indication': # the remote requires an ack. send one.
                ...
                ack = Message(name='Ack:Indication', hash=H(
                    data))    # hash of original received packet
                # yield multitask.send(self.sock, str(ack), remote) # send the NetworkAck message
                self.sock.sendto(str(ack), (
                    remote.ip, remote.port)) # send the NetworkAck message
            msg[
                'remote'] = remote # put remote as an attribute in msg before putting on queue.
            yield self.put(
                msg)    # put the parsed msg so that other interested party may get it.
...
class RoutingTable(list):
    ...
    def __init__(self, node, columns=16, scale=0.9):
        ...
        self.rows = (
                        Hsize * 8) / self.bitsPerDigit # modulus==2**(Hsize*8) and rows*columns==modulus
        ...
        for row in xrange(0,
                          self.rows): # initialize the two-dimensional list (array)
            self.append([entry if row == col else None for col in
                                                                           ...
                                                                           map(
                                                                               ...
                                                                               filter(
                                                                                   ...
                                                                                   xrange(
                                                                                       0,
                                                                                       self.columns))))),
                                           xrange(0, self.highestLevel + 1)))
        return '<RoutingTable node=%r size=%r levels=%r highestLevel=%r %s>' % (
            self.node, self.size, self.rows, self.highestLevel, entries)

    def __contains__(self, node):
        '''Check if the node exists in the routing table.'''
        dest = self.guid2digits(node.guid)
        first = self.firstdiff(dest)
        return first != self.rows and self.primary(first, dest[
            first]) and self.primary(first, dest[first]) == node
        ...
        if self[first][dest[first]] and self[first][dest[first]].node == node:
            ...
            if first == self.highestLevel:
                ...
                for i in xrange(first, -1, -1):
                    j = find(lambda y: self[i][y] and self[i][y] != self.entry,
                             xrange(0, self.columns))
    ...
    def randomLevelNode(self, level):
        ...
        choices = set(filter(lambda y: y and y != self.entry,
                             (self[level][x] for x in xrange(0, self.columns))))
    ...
    def weightedRandomLevel(self):
        ...
        r = random.randint(1, (self.highestLevel + 1) * (
            self.highestLevel + 2) / 2) # random in range 1 to sum
        level = 0
    ...
    def weightedRandomKey(self):
        ...
        digits = map(lambda x: self.digits[x] if x < level else (
            value if x == level else random.randint(0, self.columns - 1)),
                     xrange(0, self.rows))
...
def _testRoutingTable():
    ...
    for port in xrange(0, 100):
        rt.append(RoutingTable(
            node=Node(ip='0.0.0.0', port=port, type=socket.SOCK_DGRAM,
                      guid=H('0.0.0.0:' + str(port)))))
...
class LeafSet(object):
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            ...
            return self._preds[-1] if len(self._preds) > 0 else self.node
        elif index == 'high':
            return self._succs[-1] if len(self._succs) > 0 else self.node
        elif index > 0:
            return index <= len(self._succs) and self._succs[index - 1] or None
        elif index < 0:
            return -index <= len(self._preds) and self._preds[
                -index - 1] or None
    ...
    def __contains__(self, item):
        ...
        if isinstance(item, Node):
            return find(lambda x: x.guid == item.guid,
                        self._preds + self._succs) >= 0 if self.node.guid != item.guid else False
        else:
            return inrange(self['low'].guid, self.node.guid, item) or inrange(
                self.node.guid, self['high'].guid, item)
    ...
    def random(self):
        ...
        return index < len(self) and self._preds[index] or self._succs[
            index - len(self._preds)]
    ...
    def __repr__(self):
        ...
        return '<LeafSet %s>' % ('\n    '.join(map(repr, self.sorted)) if len(
            self._preds) > 0 else 'empty')

    def closest(self, guid, ignore=()):
        '''Return closest node to the given guid, ignoring elements in ignore (set).'''
        mn, md = self.node, distance(self.node.guid,
                                     guid) # result node and distance
        for n in filter(lambda x: x not in ignore, (self._preds + self._succs)):
            d = distance(n.guid,
                         guid) # distance with node n, check if it smaller than md.
            if d < md or d == md and n != mn and inrange(mn.guid, n.guid,
                                                         guid) and not inrange(
                    n.guid, mn.guid, guid):
    ...
    def intermediates(self, guid):
        ...
        p = find(lambda x: x.guid == guid,
                 self._preds)  # the guid is found in leaf set
        s = find(lambda x: x.guid == guid,
                 self._succs)  # the guid is found in leaf set
        result = (filter(lambda x: inrange(guid, self.node.guid, x.guid),
                         self._preds[0:p]) if p >= 0 else []) + \
                 (filter(lambda x: inrange(self.node.guid, guid, x.guid),
                         self._succs[0:s]) if s >= 0 else [])
        return result if len(result) > 0 else None

    def replicas(self, guid, desired):
        '''Return a set of nodes that should have replicas for the given guid.'''
        desired, result = min(desired, len(self._preds) + len(
            self._succs)), set() # result is set of nodes
        assert desired % 2 == 0 # must be even number since preds and succs are equal lengths
        if len(self._preds) == 0:
            result.add(self.node) # self node is a must
        else: # get the closest node in pred and succ and add all between those and us.
            m = distance(self.node.guid, guid)
            m, closest, ignore = reduce(
                lambda a, b: (b, a[2] - 1, a[2] - 1) if b < a[0] else (
                    a[0], a[1], a[2] - 1),
                map(lambda x: distance(x.guid, guid), self._preds), (m, 0, 0))
            m, closest, ignore = reduce(
                lambda a, b: (b, a[2] + 1, a[2] + 1) if b < a[0] else (
                    a[0], a[1], a[2] + 1),
                map(lambda x: distance(x.guid, guid), self._succs),
                (m, closest, 0))
            half = desired / 2
            start = 0
            if self.overlap or (0 - closest != len(self._preds)) and (
                    closest != len(self._succs)):
                if closest == 0:
                    start = (closest - half) if inrange(self._preds[0].guid,
                                                        self.node.guid,
                                                        guid) else (
                        closest - half + 1)
                elif closest < 0:
                    start = (closest - half + 1) if inrange(
                        self._preds[-1 * closest - 1].guid, self.node.guid,
                        guid) else (closest - half)
                else:
                    start = (closest - half) if inrange(self.node.guid,
                                                        self._succs[
                                                            closest - 1].guid,
                                                        guid) else (
                        closest - half + 1)
                ...
                for index in xrange(start, stop):
                    ...
                    elif index < 0 and 1 - index < len(self._preds):
                        result.add(self._preds[-index - 1])
                    elif index > 0 and index - 1 < len(self._succs):
                        result.add(self._succs[index - 1])
        return result
    ...
    def promising(self, node):
        ...
        if len(
                self._preds) == 0: return True # everything is promising in empty list
        if find(lambda x: x.guid == node.guid,
                self._preds + self._succs) >= 0: return False # exists
        if len(self._preds) < self.maxsize or len(
                self._succs) < self.maxsize: return True # can always accomodate until max size is reached
        return find(lambda x: inrange(x.guid, self.node.guid, node.guid),
                    self._preds) >= 0 or \
               find(lambda x: inrange(self.node.guid, x.guid, node.guid),
                    self._succs) >= 0
    ...
    def add(self, node):
        ...
        if len(self._preds) == 0:
            self._preds.append(node);
            self._succs.append(node);
            self.overlap = True # preds and succs overlap
            return self.node
        ret = None

        if findNode(node.guid, self._preds) < 0: # not found in preds
            for i in xrange(0, len(self._preds) + 1):
                if i < len(self._preds) and inrange(self._preds[i].guid,
                                                    self.node.guid,
                                                    node.guid) or i == len(
                        self._preds) < self.maxsize:
                    self._preds.insert(i, node)
                    ret = len(
                        self._preds) > self.maxsize and self._preds.pop() or self.node
                    break
        if findNode(node.guid, self._succs) < 0: # not found in preds
            for i in xrange(0, len(self._succs) + 1):
                if i < len(self._succs) and inrange(self.node.guid,
                                                    self._succs[i].guid,
                                                    node.guid) or i == len(
                        self._succs) < self.maxsize:
                    self._succs.insert(i, node)
                    ret = len(
                        self._succs) > self.maxsize and self._succs.pop() or self.node
                    break
    ...
    def remove(self, node):
        ...
        p, s = find(lambda x: x.guid == node.guid, self._preds), find(
            lambda x: x.guid == node.guid, self._succs)
        if p >= 0: del self._preds[p]
        if s >= 0: del self._succs[s]
        if _debug: print 'LeafSet.remove p=%d, s=%d, node=%r, len=%d' % (
            p, s, node, len(self._preds))
        if p >= 0 or s >= 0:
            self._set = self._list = None
            if len(self._preds) != len(self._succs):
                if _debug: print 'LeafSet.remove adjusting lengths', len(
                    self._preds), len(self._succs)
                self.overlap = True
            else:
                self._updateOverlap()
            if self.overlap: # overlaps then add all succs and preds to mutual lists
                for n in self._preds + self._succs:
                    if hasattr(self, '_origadd'):
                        self._origadd(n)
                    else:
                        self.add(n)
            assert len(self._preds) == len(self._succs)
    ...
    def _adjustLength(self):
        ...
        if _debug: print 'LeafSet.add adjusting lengths', len(self._preds), len(
            self._succs)
        ...
        while len(self._preds) != len(self._succs):
            if index < 0:
                self._preds.append(
                    self._succs[-i]) # preds < succs. Copy succs to preds
            else:
                self._succs.append(self._preds[-i])
            i = i + 1
            self.overlap = True
...
class NodeCache(object):
    ...
    def add(self, elem):
        ...
        if self.maxsize > 0:
            if elem.guid in self.dict: # exists, just update the recent list by moving elem to front.
                elem0 = self.dict[
                    elem.guid] # this is the elem present in sorted or recent
                s, r = self.sorted.index(elem0), self.recent.index(
                    elem0) # index in sorted and recent lists
                del self.recent[r] # move it to front of recent
                self.sorted[
                    s] = elem # just in case elem0 is different from elem
            else: # need to add to self set, and sorted and recent lists.
                if len(
                        self.dict) >= self.maxsize: # need to make room by discarding the least recently recent elem
                    self.remove(self.recent[
                        -1]) # this will remove from all lists and set
                s = find(lambda x: elem.guid < x.guid,
                         self.sorted) # insert in sorted list
                self.sorted.insert(s if s >= 0 else len(self.sorted), elem)
        self.dict[
            elem.guid] = elem          # just in case, override previous elem0
    ...
    def remove(self, elem):
        ...
        if elem.guid in self.dict:
            elem = self.dict[
                elem.guid] # this is the object that is present in sorted or recent
    ...
    def closest(self, guid):
        ...
        if len(self) > 0:
            if guid in self.dict: return self.sorted[
                self.dict[guid][0]] # found an exact match, return it
            first = self.sorted[0]
            last = self.sorted[-1]
            if guid < first.guid or guid > last.guid: # not in range, return either first or last
                return distance(guid, first.guid) < distance(guid,
                                                             last.guid) and first or last
            else:
    ...
    def __repr__(self): # print out a printable representation
        return '<NodeCache len=%r\n    sorted=%r\n    recent=%r>' % (
            len(self), repr(map(lambda x: x.guid, self.sorted)),
            repr(map(lambda x: x.guid, self.recent)))
...
class LatencyTable(dict):
    ...
    def isActive(self, node, validity=5.0):
        return (self[node][1] - time.time() < validity) if (
            node in self) else False
...
class DownSet(set):
    ...
    def remove(self, item):
        set.discard(self,
                    item) # always call discard to disable exception if not found
...
class Router(object):
    ...
    def __init__(self, net, **kwargs):
        ...
        self.name = 'Router[%d]' % (Router.count)
        ...
        self.rrt = kwargs.get('rrt',
                              ReverseRoutingTable()) # reverse routing table is actually a set
        ...
        self.pdown = kwargs.get('pdown',
                                PossiblyDownSet())   # set of Node that are possibly down
        self.lt = kwargs.get('lt',
                             LatencyTable())  # table with index Node and value (latency, last-activity-time)
        ...
        if not hasattr(self.ls,
                       '_origremove'): self.ls._origremove = self.ls.remove
        self.ls.remove = self.lsremove #new.instancemethod(lsremove, self.ls, LeafSet)
        if not hasattr(self.pdown,
                       '_origadd'): self.pdown._origadd = self.pdown.add
    ...
    def initialize(self): # router has joined the DHT. start other tasks.
        ...
        for gen in [self.updateleafset(), self.partitioncheck(),
                    self.updatetablenear(), self.updatetablefar()]:
            self._gens.append(gen)
            multitask.add(gen)
        return self
    ...
    def handler(self):
        ...
        supported = ['Route:Request', 'Neighbor:Indication', 'Ping:Request',
                     'RoutingTable:Request', 'LeafSet:Request', 'Join:Request',
                     'Lookup:Request', 'LeafSet:Indication']
        while True: # schedule process message, but don't wait to start new before processing previous
            ...
            if msg.name == 'Route:Request':
                multitask.add(self.routehandler(msg))
            elif msg.name == 'Neighbor:Indication':
                multitask.add(self.neighborhandler(msg))
            elif msg.name == 'RoutingTable:Request':
                multitask.add(self.rthandler(msg))
            elif msg.name == 'LeafSet:Indication':
                multitask.add(self.leafsethandler(msg))
            elif msg.name == 'Join:Request':
                multitask.add(self.joinhandler(msg))
            elif msg.name == 'Lookup:Request':
                multitask.add(self.lookuphandler(msg))
            elif msg.name != 'Ping:Request' and _debug:
                print 'invalid message name', msg.name
    ...
    def send(self, msg, node, timeout=None, ping=False):
        ...
        if pinging: # process the response only when timeout is specified.
            ...
            if result:
                ...
                self.lt.add(
                    node) # check if we can add it to our data structures
            elif not msg.second: # failed to send first ping
                self.cache.remove(node)
                self.lt.remove(node)
                multitask.add(self.pdown.add(
                    node)) # in background perform second ping, and on confirmation remove the node.
    ...
    def periodicping(self, timeout=180):
        ...
        while True:
            ...
            all = set(
                filter(lambda x: x not in self._pings and x not in self.pdown,
                       all)) # remove those that are down or we recently pinged
            msg = Message(name='Ping:Request')
            for node in all:
    ...
    def rtadd(self, node, *args, **kwargs):
        ...
        if node not in rt:
            if node not in self.lt: # no information about the node, ping it.
                ...
                if result:
                    if self.node != result: multitask.add(self.net.send(
                        Message(name='Neighbor:Indication', node=self.node,
                                add=False), node=result))
                    multitask.add(self.net.send(
                        Message(name='Neighbor:Indication', node=self.node,
                                add=True), node=node))
                node = result
        return node
            ...
            if len(self.ls) == 0:
                ...
                for row in xrange(rt.rows):
                    for col in xrange(rt.columns):
                        if rt[row][col] and rt.node != rt[row][
                            col].node and node != rt[row][col]:
    ...
    def pdownadd(self, node):
        ...
        if (node not in self.pdown) and (
                    (node in self.rt) or (node in self.ls) or (
                    node in self.rrt)): # otherwise we don't care
            ...
            if not result: # node is down, remove from data structures
                ...
                self.down.add(node)
                for ds in (self.lt, self.rt, self.ls,
                           self.rrt): # must remove from rt before ls, because ls.remove may add into ls from rt if ls is empty
                    ds.remove(node)
    ...
    def join(self, bs, cached=True, timeout=30):
        ...
        yield self.net.send(
            Message(name='Join:Request', returnaddr=self.node, path=[]),
            node=bs)
        msg = yield self.net.get(lambda x: x.name == 'Join:Response',
                                 timeout=timeout)
        if msg and len(msg.path) > 0:
            root = msg.path[
                -1] # last node in the path is the root for this node's guid
            self.cache.add(root)
            for node in [root] + msg.leafset:
                if node != self.node:
                    self.ls.add(node) # update our leafset
                    if not self.initialized: self.rt.add(
                        node) # and routing table
    ...
    def joinhandler(self, msg):
        ...
        if findNode(self.node.guid,
                    msg.path) >= 0: return # check if we have a routing loop, then don't process.
        next = self.nextHop(msg.returnaddr.guid,
                            useCache=False) # find next hop for source node; don't use node cache.
        if not next or next == self.node or next == msg.returnaddr: # invalid next or matches local; send response.
            ...
            yield self.net.send(
                Message(name='Join:Response', path=msg.path + [self.node],
                        leafset=old), node=msg.returnaddr)
        else:
            del msg['remote']
            msg.path.append(self.node);
            msg['inbound'] = False # add this node in the path
            result = yield self.net.send(msg, node=next,
                                         timeout=5)# and proxy the message to next hop
            if result: self.down.remove(
                next)  # request was sent successfully, remove next from down nodes.
            ...
            if len(self.bs) > 0 and not self.initialized:
                ...
                if node:
                    ...
                    if not result:
                        self.bs.remove(node)
                        if len(
                                self.bs) == 0: self.initialized = False # no more connected to bootstrap
    ...
    def nextHop(self, guid, useCache=False):
        ...
        if guid in self.ls: # guid is in leaf-set range, find the closest node in leaf set
            ...
            if cached and distance(cached.guid, guid) < distance(node.guid,
                                                                 guid): node = cached
            return node
    ...
    def route(self, guid, payload):
        ...
        if not next or next == self.node: # deliver to this node
            ...
            msg = Message(name='Route:Request', src=self.node.guid, dest=guid,
                          payload=payload)
    ...
    def routehandler(self, msg):
        ...
        if not next or next == self.node:
            ...
            if 'remote' in msg: del msg[
                'remote'] # remove additional parameter that was added by network
    ...
    def neighborhandler(self, msg):
        ...
        if msg.add == True:
            self.rrt.add(msg.remote)
        else:
            self.rrt.remove(msg.remote)
        yield
    ...
    def leafsethandler(self, msg):
        ...
        if msg.wantreply and msg.remote != self.node:
            yield self.send(Message(name='LeafSet:Indication', node=self.node,
                                    leafset=list(self.ls.set)), node=msg.remote,
                            timeout=5, ping=True)
        yield
    ...
    def updateleafset(self, timeout=40):
        ...
        while True:
            ...
            if node and node not in self.pdown:
                yield self.send(
                    Message(name='LeafSet:Indication', node=self.node,
                            leafset=list(self.ls.set), wantreply=True),
                    node=node, timeout=5, ping=True)
    ...
    def rthandler(self, msg):
        ...
        msg = Message(name='RoutingTable:Response', node=self.node,
                      neighbors=nodes)
    ...
    def updatetablenear(self, timeout=100):
        ...
        while True:
            ...
            if len(self.rt) > 0 and node and node not in self.pdown:
                yield self.send(Message(name='RoutingTable:Request',
                                        level=self.rt.weightedRandomLevel),
                                node=node, timeout=5, ping=True)
                msg = yield self.net.get(
                    lambda x: x.name == 'RoutingTable:Response', timeout=5)
                if msg:
                    self.cache.add(node)
                    nodes = filter(lambda
                                           x: x.guid < Hmod and x != self.node and x not in self.ls and x not in self.rt,
                                   msg.neighbors)
                    if nodes:
                        notadded = []
                        for x in nodes:
                        ...
                        if notadded:
                            ...
                            if node != x: # do a ping to random entry so that we may discover a better node
                                yield self.send(Message(name='Ping:Request'),
                                                node=x, timeout=5)
    ...
    def lookuphandler(self, msg):
        ...
        self.cache.add(node)
        yield self.net.send(
            Message(name='Lookup:Response', guid=msg.guid, node=self.node),
            node=node)
    ...
    def updatetablefar(self, timeout=200):
        while True:
            ...
            if guid:
                yield self.route(guid=guid,
                                 payload=Message(name='Lookup:Request',
                                                 guid=guid,
                                                 returnaddr=self.node))
                msg = yield self.net.get(lambda x: x.name == 'Lookup:Response',
                                         timeout=5)
                if msg:
...
class Key(object):
    def __init__(self, **kwargs):
        ...
        if 'value' in kwargs: # need to parse
            ...
            t1, t2, e1, e2, put, guid, hash, nonce, owner = struct.unpack(_fmt,
                                                                          self.str)
            self.time, self.expires = long(t1 * (2 ** 32) + t2), long(
                e1 * (2 ** 32) + e2)
            self.put = (put != 0)
            self.guid, self.hash, self.nonce, self.owner = bin2int(
                guid), bin2int(hash), bin2int(nonce), bin2int(owner)
        else: # need to construct from individual fields
            for n in (
                'time', 'expires', 'put', 'guid', 'hash', 'nonce', 'owner',
                'client'):
                exec 'self.%s = kwargs.get("%s", None)' % (n, n)
            self.time = long(self.time);
            self.expires = long(self.expires)
            t1, t2, e1, e2 = int(self.time / (2 ** 32)), int(
                self.time % (2 ** 32)), int(self.expires / (2 ** 32)), int(
                self.expires % (2 ** 32))
            put = (self.put and 1 or 0)
            guid, hash, nonce, owner = int2bin(self.guid), int2bin(
                self.hash), int2bin(self.nonce), int2bin(self.owner)
            self.str = struct.pack(_fmt, t1, t2, e1, e2, put, guid, hash, nonce,
                                   owner)
            if len(
                    self.str) != 97: raise ValueError, 'invalid length of the key %d' % (
                len(self.str))
        self.index = struct.pack(_fmtindex, int2bin(self.guid),
                                 int2bin(self.hash), int2bin(self.nonce),
                                 int2bin(self.owner))

    def __repr__(self):
        return '<Key time=%r expires=%r put=%r guid=%r hash=%r nonce=%r owner=%r>' % (
            self.time, self.expires, self.put, self.guid, self.hash, self.nonce,
            self.owner)
    ...
    def __cmp__(self, other):
        return 0 if id(self) == id(other) else cmp(self.str, other.str)
...
class Value(object):
    def __init__(self, **kwargs):
        if 'raw' in kwargs:
            raw = kwargs.get('raw')
            l, = struct.unpack('!H', raw[:2]);
            self.value, raw = raw[2:2 + l], raw[2 + l:]
            l, = struct.unpack('!H', raw[:2]);
            self.hash, raw = bin2int(raw[2:2 + l]), raw[2 + l:]
            l, = struct.unpack('!H', raw[:2]);
            self.Kp, raw = PublicKey(value=raw[2:2 + l]), raw[2 + l:]
            l, = struct.unpack('!H', raw[:2]);
            self.sigma, raw = raw[2:2 + l], raw[2 + l:]
        else:
            for n in ('value', 'hash', 'Kp', 'sigma'):
                exec 'self.%s = kwargs.get("%s", None)' % (n, n)
        if isinstance(self.value, long): print 'Incorrect Value(value=%r)' % (
            self.value)

    def __repr__(self):
        return '<value value-len=%d hash=%r Kp=%r sigma=%r, value=%r>' % (
            len(self.value) if self.value else 0, self.hash, self.Kp, self.sigma,
            self.value)
    ...
    def __str__(self):
        value, hash, Kp, sigma = str(self.value), int2bin(self.hash), str(
            self.Kp), str(self.sigma)
        return struct.pack('!H', len(value)) + value + struct.pack('!H', len(
            hash)) + hash + struct.pack('!H', len(Kp)) + Kp + struct.pack('!H',
                                                                          len(
                                                                              sigma)) + sigma
...
class Database(object):
    ...
    def put(self, key, value):
        ...
        if toput:
            ...
            self._put(key, value)
        if _debug: print 'db.put(key=%r,value=%r) returns %r' % (
            key, value, result)
        return result
    ...
    def _cleanup(self, guid):
        ...
        for oi in o.keys():
            ...
            for vi in v.keys():
                i = v.get(vi, None)
                d = self._data[i] if (
                    i is not None and i in self._data) else None
    ...
    def _remove(self, key, value):
        ...
        v = self._data[i]
        if v is None: del self._uniq[key.index]; return (
            True, (None, None, None, None))
        oldkey, oldvalue = i, v
        toput = True
        toremove = not True
        if key == oldkey:
            toput = False; result = (None, None, None, None)
        else:
            if key.put and oldkey.put:
                if key.expires <= oldkey.expires:
                    toput = False; result = (key, value, None, None)
                else:
                    toremove = True; result = (oldkey, oldvalue, None, None)
            elif not key.put and not oldkey.put:
                if key.expires <= oldkey.expires:
                    toput = False; result = (None, None, key, value)
                else:
                    toremove = True; result = (None, None, oldkey, oldvalue)
            elif key.put and not oldkey.put:
                toput = False; result = (key, value, None, None)
            elif not key.put and oldkey.put:
                toremove = True;  result = (oldkey, oldvalue, None, None)
        if toremove:
            del self._data[i]
    ...
    def _put(self, key, value):
        ...
        if key.guid in self._guid:
            o = self._guid[key.guid]
        else:
            o = self._guid[key.guid] = dict()
    ...
    def get(self, guid, owner=None, maxvalues=32):
        ...
        o = self._guid[guid]
        if owner is None:
            v = set(sum(map(lambda x: x.values(), o.values()), []))
        else:
            v = set(sum(o[owner].values(), [])) if owner in o else set()
        result = map(lambda y: (y, self._data[y]),
                     filter(lambda x: x in self._data and x.put, v))
        if _debug: print 'db.get(guid=%r,owner=%r,maxvalues=%r) returns %r' % (
            guid, owner, maxvalues, result)
        return result
    ...
    def getkeys(self, low, high):
        ...
        if low <= high:
            guids = sorted(
                filter(lambda x: low <= x <= high, self._guid.keys()))
        else:
            guids = sorted(
                filter(lambda x: x >= low, self._guid.keys())) + sorted(
                filter(lambda x: x <= high, self._guid.keys()))
        now = time.time()
        return filter(lambda y: y.expires >= now, sum(map(
            lambda z: sum(map(lambda w: w.values(), self._guid[z].values()),
                []), guids), []))
...
def _testDatabase():
    db = Database()
    k1 = Key(time=time.time(), expires=time.time() + 60, put=True, guid=10,
             hash=10, nonce=10, owner=10)
    v1 = Value(value='kundan', Kp='empty', sigma='empty')
    k2 = Key(time=time.time(), expires=time.time() + 60, put=True, guid=10,
             hash=10, nonce=11, owner=10)
    ...
    k11 = Key(time=time.time(), expires=time.time() + 120, put=True, guid=10,
              hash=10, nonce=10, owner=10)
...
class Range(object):
    ...
    def __init__(self, low, high):
        if low is None or high is None: raise ValueError(
            'Invalid Range[%r, %r]' % (low, high))
    ...
    def __cmp__(self, other):
        if id(self) == id(other):
            ...
            return cmp(self.low, other.low) or cmp(self.high, other.high)
...
class Ranges(dict):
    ...
    def add(self, range):
        if range not in self:
            self[range] = range
            s = find(lambda x: range.low < x.low or (
                range.low == x.low and range.high < x.high),
                     self.sorted) # insert in sorted list
            self.sorted.insert(s if s >= 0 else len(self.sorted), range)
        else:
            s = self.sorted.index(self[range])
            self.sorted[s] = self[
                range] = range # update the value with new range
    ...
    def synched(self, guid, node, ls):
        ...
        if node:
            return (s >= 0) and (node in self.sorted[s].sync) or False
        elif not ls or len(ls) == 0:
            return True # always synced if no leafset
        else:
            return (s >= 0) and (len(self.sorted[s].sync) > len(
                ls)) or False # return true if we synched with more than half of leaf set
    ...
    def invalidate(self, guid):
        ...
        try:
            for range in filter(lambda x: inrange(x.low, x.high, guid),
                                self.sorted):
                range.hash = None
        except:
...
class Storage(object):
    ...
    def __init__(self, net, router, replicas=8, required=5):
        ...
        self.replicas, self.required = (
                                           replicas / 2) * 2, required # make it even
    ...
    def start(self):
        for gen in [self.handler(), self.periodicantropy(),
                    self.periodicdiscard()]:
            self._gens.append(gen)
            multitask.add(gen)
        return self
    ...
    def handler(self):
        supported = ['Put:Request', 'Get:Request', 'Replicate:Request',
                     'ReplicaSet:Request', 'Hash:Request', 'Keys:Request',
                     'Data:Request']
        while True:
            msg = yield self.net.get(lambda x: x.name in supported)
            if _debug: print 'storage.handler name=', msg.name
            if msg.name == 'Put:Request':
                multitask.add(self.puthandler(msg))
            elif msg.name == 'Get:Request':
                multitask.add(self.gethandler(msg))
            elif msg.name == 'Replicate:Request':
                multitask.add(self.replicatehandler(msg))
            elif msg.name == 'Hash:Request':
                multitask.add(self.hashhandler(msg))
            elif msg.name == 'Keys:Request':
                multitask.add(self.keyshandler(msg))
            elif msg.name == 'Data:Request':
                multitask.add(self.datahandler(msg))
            elif msg.name == 'ReplicaSet:Request':
                multitask.add(self.rshandler(msg))
            elif _debug:
                print 'invalid message', msg.name
    ...
    def puthandler(self, msg, timeout=60, defaultTTL=600):
        ...
        try:
            ...
            key = Key(time=msg.time, expires=msg.expires, put=msg.put,
                      guid=msg.dest, hash=hash, nonce=msg.nonce, owner=owner)
            ...
            msg[
                'guid'] = msg.dest  # TODO: this was added because guid was needed in replicate request
            global _seq;
            _seq = _seq + 1;
            msg.seq = _seq
            ...
            while len(replicas) > (
                    self.replicas - self.required): # wait for more responses
                resp = yield self.net.get(lambda
                                                  x: x.name == 'Replicate:Response' and x.seq == msg.seq,
                                          timeout=(
            ...
            yield self.net.send(
                Message(name='Put:Response', seq=seq, result=True),
                node=msg.src)
            ...
            try:
                yield self.net.send(
                    Message(name='Put:Response', seq=msg.seq, result=False,
                            error=str(E)), node=msg.src)
            except:
                pass
        except:
            traceback and traceback.print_exc()
    ...
    def replicatehandler(self, msg):
        ...
        try:
            ...
            key = Key(time=msg.time, expires=msg.expires, guid=msg.guid,
                      hash=hash, nonce=msg.nonce, owner=owner)
            ...
            yield self.net.send(Message(name='Replicate:Response', seq=msg.seq),
                                node=msg.remote)
            ...
            try:
                yield self.net.send(
                    Message(name='Replicate:Error', seq=msg.seq, error=str(E)),
                    node=msg.remote)
            except:
                pass
        except:
    ...
    def gethandler(self, msg):
        ...
        keyvals = yield self.db.get(guid=msg.dest, owner=msg.owner,
                                    maxvalues=msg.maxvalues)
        vals = [v for k, v in keyvals]
        keyss = [k for k, v in keyvals]
        yield self.net.send(
            Message(name='Get:Response', seq=msg.seq, guid=msg.dest, vals=vals,
                    keyss=keyss),
            node=msg.src) # send response directly to the source
    ...
    def leafsetchanged(self):
        ...
        if ls.coversAll(self.replicas):
            ...
            self.low, self.high = ls._preds[size - 1].guid, (
                ls._succs[size - 1].guid - 1)
    ...
    def periodicantropy(self, timeout=3, reset=30):
        ...
        while True:
            ...
            hash = H(
                ''.join(map(str, keys))) # hash of all the keys in the range
            seq = _seq = _seq + 1
            yield self.net.send(
                Message(name='Hash:Request', seq=seq, low=range.low,
                        high=range.high, hash=hash), node=node)
            msg = yield self.net.get(
                lambda x: x.name == 'Hash:Response' and x.seq == seq,
                timeout=(reset - time.time() + start))
            if not msg: # timedout
            ...
            if msg.hash == hash: # we are in sync
                ...
                seq = _seq = _seq + 1
                yield self.net.send(
                    Message(name='Keys:Request', seq=seq, low=range.low,
                            high=range.high), node=node)
                msg = yield self.net.get(
                    lambda x: x.name == 'Keys:Response' and x.seq == seq,
                    timeout=(reset - time.time() + start))
                ...
                try:
                    remotekeys = filter(
                        lambda y: find(lambda x: str(x) == str(y), keys) < 0,
                        msg.keyss) # all keys in msg which are not in local keys
                except:
                ...
                for key in remotekeys: # for each unmatched key, synchronize the data.
                    seq = _seq = _seq + 1
                    yield self.net.send(
                        Message(name='Data:Request', seq=seq, key=key),
                        node=node)
                    msg = yield self.net.get(
                        lambda x: x.name == 'Data:Response' and x.seq == seq,
                        timeout=(reset - time.time() + start))
                    if msg and msg.key.expires > time.time(): # not yet expired key
                        ...
                        if valid:
                            ...
                            if msg.key.put:
                                value = Value(value=msg.value, Kp=msg.Kp,
                                              sigma=msg.sigma)
                            else:
                                value = Value(hash=msg.hash, Kp=msg.Kp,
                                              sigma=msg.sigma)
    ...
    def unsynched(self, range, node):
        ...
        if range in self.ranges:
            try:
                del self.ranges.get(range).sync[node]
            except KeyError:
    ...
    def getSharedRanges(self, guid):
        ...
        for i in xrange(0, len(ls)):
            if guid == ls._succs[i].guid:
                low, high = (0 if (i == len(ls) - 1) else (
                    -len(ls) + i + 1)), len(ls)
                break
            elif guid == ls._preds[i].guid:
                low, high = -len(ls), (
                    0 if (i == len(ls) - 1) else (len(ls) - i - 1))

        guid = self.node.guid
        for j, k in map(lambda x: (x, x + 1), xrange(low, high)):
            a = guid if j == 0 else (
                ls._preds[-j - 1].guid if j < 0 else ls._succs[j - 1].guid)
            b = guid if k == 0 else (
                ls._preds[-k - 1].guid if k < 0 else ls._succs[k - 1].guid)
            b = b - 1
            if b == -1: b = (Hmod - 1)
            ret.add(Range(a, b))
        return ret
    ...
    def hashhandler(self, msg):
        ...
        ranges = self.getSharedRanges(node.guid)
        if len(self.ls) == 0 or len(ranges) == 0 or (
                range not in ranges): # not a valid range with this node
            yield self.net.send(Message(name='Hash:Response', seq=msg.seq,
                                        error='invalid range'), node=node)
            return
        keys = yield self.db.getkeys(low=range.low, high=range.high)
        if range not in self.ranges:
            self.ranges.add(range)
        ...
        yield self.net.send(
            Message(name='Hash:Response', seq=msg.seq, hash=hash), node=node)
    ...
    def keyshandler(self, msg):
        ...
        if range in self.ranges:
            ...
            yield self.net.send(
                Message(name='Keys:Response', seq=msg.seq, keyss=keys),
                node=node)
        else:
            yield self.net.send(Message(name='Keys:Response', seq=msg.seq,
                                        error='invalid range'), node=node)
    ...
    def datahandler(self, msg):
        ...
        if value.value is not None:
            yield self.net.send(
                Message(name='Data:Response', seq=msg.seq, key=msg.key,
    ...
    def periodicdiscard(self, timeout=5):
        ...
        while True:
            yield randomsleep(timeout)
            if self.ls.coversAll(
                    self.replicas): continue # no need to discard if we cover all
            keys = yield self.db.getkeys(low=self.ls['high'].guid, high=self.ls[
                'low'].guid) # get keys in inverse range
            keys = filter(lambda x: not inrange(self.low, self.high, x.guid),
                          keys)
            for key in keys:
                seq = _seq = _seq + 1
                yield self.route(guid=key.guid,
                                 payload=Message(name='ReplicaSet:Request',
                                                 seq=seq, src=self.node.guid,
                                                 dest=key.guid))
                msg = yield self.net.get(
                    lambda x: x.name == 'ReplicaSet:Response' and x.seq == seq,
                    timeout=timeout)
                replicas = msg.nodes
                if replicas:
                    ...
                    seq = _seq = _seq + 1
                    yield self.net.send(
                        Message(name='Replicate:Request', time=key.time,
                                seq=seq, guid=key.guid, value=value.value,
                                hash=key.hash, nonce=key.nonce,
                                expires=key.expires, put=key.put,
                                owner=key.owner, Kp=value.Kp,
                                sigma=value.sigma), node=node)
                    msg = yield self.net.get(lambda
                            x: x.name == 'Replicate:Response' and x.seq == seq)
    ...
    def rshandler(self, msg):
        ...
        yield self.route(guid=msg.src,
                         payload=Message(name='ReplicaSet:Response',
                                         seq=msg.seq, nodes=replicas))


    def replicaNodes(self, guid):
        ls, size, replicas = self.ls, len(self.ls), set()
        if size > 0:
            if _debug: print 'replicaNodes', ls._preds[
                0].guid, self.node.guid, guid
            for i in xrange(
                    (size - 1) if inrange(ls._preds[0].guid, self.node.guid,
                                          guid) else (size - 2), -1, -1):
                replicas.add(ls._preds[i])
            for i in xrange(
                    (size - 1) if inrange(ls._succs[0].guid, self.node.guid,
                                          guid) else (size - 2), -1, -1):
...
def put(net, guid, value, nonce, expires, Ks=None, put=True, timeout=30,
        retry=7):
    ...
    global _seq
    seq = _seq = _seq + 1
    request = Message(name='Put:Request', date=time.time(), seq=seq,
                      src=net.node, dest=guid, nonce=nonce, expires=expires,
                      put=put, \
                      value=str(value), hash=H(str(value)),
                      Kp=Ks and extractPublicKey(Ks) or None, \
                      sigma=sign(Ks, H(
                          str(guid) + str(value) + str(nonce) + str(
                              expires))) if Ks else None)

    while retry > 0:
        yield net.put(Message(name='Route:Request', src=net.node, dest=guid,
                              payload=request), timeout=5)
        response = yield net.get(timeout=timeout, criteria=lambda
                x: x.seq == seq and x.name == 'Put:Response') # wait for response
        if response:
            raise StopIteration(response.result)
        else:
            retry = retry - 1
...
def remove(net, guid, value, nonce, expires, Ks=None, timeout=30, retry=7):
    ...
    result = yield put(net, guid, value, nonce, expires, Ks, False, timeout,
                       retry)
    raise StopIteration(result)
...
def get(net, guid, maxvalues=16, Kp=None, timeout=5):
    ...
    global _seq
    seq = _seq = _seq + 1
    request = Message(name='Get:Request', seq=seq, src=net.node, dest=guid,
                      maxvalues=maxvalues, hash=Kp and H(str(Kp)) or None)
    ...
    while retry > 0:
        yield net.put(Message(name='Route:Request', src=net.node, dest=guid,
                              payload=request), timeout=5)
        response = yield net.get(timeout=timeout, criteria=lambda
                x: x.seq == seq and x.name == 'Get:Response') # wait for response
        if response:
            result = [(v.value, k.nonce, v.Kp, k.expires) for k, v in
                      zip(response.get('keyss', [None] * len(response['vals'])),
                          response['vals'])]
            raise StopIteration(
                result) # don't use response.values as it is a built-in method of base class dict of Message.
        else:
            retry = retry - 1
...
def _testDHT():
    ...
    yield put(net=n1, guid=H('kundan'), value='Kundan Singh',
              nonce=randomNonce(), expires=time.time() + 60, Ks=PrivateKey())
