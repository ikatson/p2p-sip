...
class AbstractAgent(object):
    ...
    def __init__(self, sipaddr=('127.0.0.1', 5062), stack=Stack):
        ...
        sock.bind(sipaddr)
        self.udp = stack(self, TransportInfo(sock))
        self.udp.sock = sock
        sock = socket.socket(type=socket.SOCK_STREAM)
        sock.bind(sipaddr)
        sock.listen(5)
        self.tcp = stack(self, TransportInfo(sock))
        self.tcp.sock = sock
    ...
    def start(self):
        """Start the agent."""
        for gen in [self._sipreceiver(self.udp),
                    self._sipreceiver(self.tcp)]: self._gens.append(
            gen); multitask.add(gen)
        return self
    ...
    def receivedRequest(self, ua, request, stack):
        ...
        try:
            if hasattr(self, handlerName) and callable(eval(
                            'self.' + handlerName)): # user has defined onINVITE, onREGISTER, etc
    ...
    def onREGISTER(self, ua, request, stack): # incoming registration
        ...
        if auth == 200:
            ...
            response.insert(Header(
                createAuthenticate(realm='localhost', domain=str(request.uri),
                                   stale=('FALSE' if auth == 401 else 'TRUE')),
                'WWW-Authenticate'), append=True)
            ua.sendResponse(response)

    def onPUBLISH(self, ua, request,
                  stack): # incoming publish is handled as register
    ...
    def onRequest(self, ua, request, stack): # any other request
        if request.Route: # if route header is present unconditionally proxy the request
            proxied = ua.createRequest(request.method, dest=request.uri,
                                       recordRoute=(request.method == 'INVITE'))
    ...
    def isProxy(self, request): # return True to proxy, False to redirect
        return request['user-agent'] and request['user-agent'].value.find(
            'X-Lite') >= 0
    ...
    def authorize(self, request, realm='localhost'):
        ...
        auths = filter(lambda x: x['realm'] == realm,
                       request.all('Authorization',
                                   'Proxy-Authorization')) # search all our authenticate headers
    ...
    def save(self, msg, uri, defaultExpires=3600):
        ...
        if uri in self.location:
            existing = self.location[uri]
        else:
            existing = self.location[
                uri] = [] # initialize that user's contacts list
        if msg.Contact and msg.first(
            ...
            for c in msg.all('Contact'): # for all contacts in the new msg
                e = now + (expires if 'expires' not in c else int(
                    c.expires)) # expiration for this contact.
                existing[:] = filter(lambda x: x[0].value.uri != c.value.uri,
                                     existing) # remove matching contacts
                existing.insert(0, (c, e)) # insert the new contact in begining
            existing[:] = filter(lambda x: x[1] > now,
                                 existing) # filter out expired contacts
    ...
    def locate(self, uri):
        ...
        now = time.time()
        existing[:] = filter(lambda x: x[1] > now,
                             existing) # remove expired headers
        for c in existing: c[0]['expires'] = str(
            int(c[1] - now)) # update the expires header with relative seconds
...
class Agent(AbstractAgent):
    ...
    def __init__(self, server=False, sipaddr=('127.0.0.1', 5062), port=0):
        ...
        self.p2p = ServerSocket(server=server,
                                port=port) # for initial testing start as bootstrap server
        self.location = None # to prevent accidental access to location dictionary

    def start(self, servers=None):
        '''Start the Agent'''
        self.p2p.start(servers=servers);
    ...
    def p2preceiver(self, p2p):
        ...
        while True:
            ...
            if hasattr(self, 'identity') and self.identity: multitask.add(
                p2phandler(sock))
    ...
    def save(self, msg, uri, defaultExpires=3600):
        ...
        if msg.Contact and msg.first(
            ...
            existing = dict(
                [(str(Header(str(x[0]), 'Contact').value.uri), x) for x in
                 existing])
...
if __name__ == '__main__':
    ...
    parser.add_option('-s', '--server', dest='server', default=False,
                      action="store_true", help='start as bootstrap node')
    parser.add_option('-p', '--sip-port', dest='sip_port', default=5062,
                      type="int",
                      help='SIP port number to listen. default is 5062.')
    parser.add_option('-P', '--p2p-port', dest='p2p_port', default=0,
                      type="int",
                      help='Preferred P2P port number to listen. If supplied, disables multicast.')
    parser.add_option('-S', '--servers', dest='servers', default=None,
                      help='list of bootstrap nodes as "ip:port,ip:port,..."')
    (options, args) = parser.parse_args()

    _debug = p2p._debug = rfc3261._debug = options.verbose

    servers = [(x.split(':', 1)[0], int(x.split(':', 1)[1])) for x in
               options.servers.split(',')] if options.servers else None

    agent = Agent(server=options.server, sipaddr=('0.0.0.0', options.sip_port),
                  port=options.p2p_port).start(servers)
    try:
        multitask.run()
